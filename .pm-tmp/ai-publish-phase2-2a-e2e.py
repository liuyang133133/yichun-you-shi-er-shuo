...
def main() -> int:
    ...
    # ---------- [B] regenerate-seo ----------
    hr("[B] regenerate-seo (admin) → 写 Post.seoMeta")
    admin_token = None
    ...
    # ---------- [C] sitemap-data 公开端点 ----------
    hr("[C] sitemap-data 公开端点 → 含 priority")
    ...
    # ---------- [D] sitemap.xml ----------
    hr("[D] sitemap.xml (Next.js) → 含 quality 权重")
    ...
    assert_true("priority=" in r.text, "含 priority=")
    print(f"  [OK] {len(r.text)} bytes")
    print(f"  [PASS D]")

    # ---------- [E] 详情页 SEO 渲染 ----------
    hr("[E] 详情页 SEO: /posts/<id> → <head> meta + JSON-LD")
    # 优先找带 seoMeta 的帖
    r = requests.get(
    ...
    # ---------- [F] 列表页 TDK ----------
    hr("[F] 列表页 TDK: /?type=house → title 含 '房屋出租'")
    r = requests.get(
    ...
    # ---------- [G] 重复发贴 ----------
    hr("[G] 重复发贴: 同 title 第二次 → 400 DUPLICATE_POST")
    ...
    err = r2.json()
    assert_eq(err.get("code"), "DUPLICATE_POST", "错误码")
    print(f"  [OK] message={err.get('message')}")
    print(f"  [PASS G]")

    print("\n" + "=" * 60)
    print("  Phase 2.2a E2E 全部通过 (7/7)")
    print("=" * 60)
    return 0
